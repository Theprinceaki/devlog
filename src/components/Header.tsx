...
type HeaderItem = { key: string; val: string; href?: string };
...
export default function Header({ rightTitle, rightItems }: HeaderProps): React.JSX.Element {
  return (
        ...
        <div className="identity">
          ...
          <div className="navRow">
            <NavLink
              ...
              className={({ isActive }) => `linkBtn navBtn ${isActive ? "navBtnActive" : ""}`}
            >
              Home
            </NavLink>
        ...
        <ul className="miniList">
          {rightItems.map((item) => (
            <li key={item.key}>
              ...
              {item.href ? (
  <a className="val linkVal" href={item.href} target="_blank" rel="noreferrer">
    {item.val}
  </a>
) : (
